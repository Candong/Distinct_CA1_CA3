...
s_rate=1/whole_data.Tinterval/1000; % khz

si={whole_data.Tinterval*s_rate*1000000};%sample interval in usec;
...
for i=1:numel(data)% For each .mat file
    %highpoint_id=find(data(i).Image_sync>4);
    %data(i).Image_sync(highpoint_id(find(diff(find(data(i).Image_sync>4==1))==1)+1))=...
        %data(i).Image_sync(highpoint_id(find(diff(find(data(i).Image_sync>4==1))==1)+3));
    ...
    % Can debug
    %jump_id=find(diff(data(i).on_t)>33==1);
    recording_f=33;
    jump_id=find(diff(data(i).on_t)>recording_f*s_rate==1);
    if ~isempty(jump_id)
        for id=1:length(jump_id)

         %for num=1:floor((data(i).on_t(jump_id(id)+1)-data(i).on_t(jump_id(id)))/33*s_rate-1)-1
          for num=1:floor((data(i).on_t(jump_id(id)+1)-data(i).on_t(jump_id(id)))/recording_f*s_rate)
           data(i).on_t(end+1)=data(i).on_t(end);
           data(i).on_t(jump_id(id)+2:end)=data(i).on_t(jump_id(id)+1:end-1);
           data(i).on_t(jump_id(id)+1)=data(i).on_t(jump_id(id))+recording_f*s_rate-1;
           if id~=length(jump_id)
           jump_id(id:end)=jump_id(id:end)+1;
           end
         end
    ...
    %jump_id=find(diff(data(i).on_t)>33==1);
    jump_id=find(diff(data(i).off_t)>recording_f*s_rate==1);
    if ~isempty(jump_id)
        for id=1:length(jump_id)

         %for num=1:floor((data(i).off_t(jump_id(id)+1)-data(i).off_t(jump_id(id)))/33*s_rate-1)-1
         for num=1:floor((data(i).off_t(jump_id(id)+1)-data(i).off_t(jump_id(id)))/recording_f*s_rate)
           data(i).off_t(end+1)=data(i).off_t(end);
           data(i).off_t(jump_id(id)+2:end)=data(i).off_t(jump_id(id)+1:end-1);
           data(i).off_t(jump_id(id)+1)=data(i).off_t(jump_id(id))+recording_f*s_rate-1;
           %if id~=length(jump_id) 
           jump_id(id:end)=jump_id(id:end)+1;
           %end
         end
...
%% real frame num
tif_frame_counts=block_duration-1;
%tif_frame_counts(2)=30000;
