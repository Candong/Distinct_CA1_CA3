...
for plane=1:3
 ...
 start_bin=[];end_bin=[];
 first25_id=setxor(n_first25_pfid{plane},n_potential_samepf{plane});
 late15_id=n_late15_pfid{plane};
 if ~isempty(first25_id)
 for k=first25_id
     load([temp behavior_filepaths{n_first25_id{plane}}]);
    for j=1:size(sig_PFs,1)
        
        if ~isempty(sig_PFs{j,k})
       start_bin=[start_bin PF_info(n_first25_id{plane}).PF_start_bin(j,k)];
       end_bin=[end_bin PF_info(n_first25_id{plane}).PF_end_bin(j,k)]; 
        end
...
count=count+1;
    ...
    PF_info(n_late15_id{plane}).delay_lap=delay_lapid{plane};
