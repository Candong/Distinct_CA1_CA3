%% Corrections constantly being made by Seetha. Use Script at your own risk
...
if is_sparse~=1
    startt = 45001;
    endt = 70000;
    Fc3_DF=data.Fc3(startt:endt,:);
    Fc2=data.Fc(startt:endt,:);
else
...
ybinned=ybinned';
E=bwlabel(double_thresh(ybinned,0.025,0.014)); %labels each traversal
w=NaN(200,2);
...
for ii = 1:numneurons
    ...
    minrate=0.3; %original =.3 %percentage of times a tranisent must occur in the PF
    baselength=0.2; %number of lowest values used to calculate a mean baseline value
    ...
    %close all
    %Save binned flouresence for all cells
    Allbinned_F{1, ii} = binMean;
end %loops through each neuron


if is_sparse==1
    save([behavior_filepaths(1:end-4), '_sparse_PF'],'sig_PFs','sig_PFs_with_noise','PF_width','PF_rate','PF_start_bins','PF_PVALS','PF_end_bins','number_of_PFs','cellbody_id')%,'-append');
else
    save([behavior_filepaths(1:end-4) '_PlaceFields'],'Allbinned_F','sig_PFs','sig_PFs_with_noise','PF_width','PF_rate','PF_start_bins','PF_PVALS','PF_end_bins','number_of_PFs')%,'-append');
end
