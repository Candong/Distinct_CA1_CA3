...
% culative
cumF_norm=zeros(size(F_norm));
cumN_norm=zeros(size(F_norm));
for i=1:size(F_norm,1)
   for j=1:size(F_norm,2) 
    cumF_norm(i,j)=sum(F_norm(i,1:j));
    cumN_norm(i,j)=sum(N_norm(i,1:j));
       
   end
end

figure;
plot(mean(cumF_norm,1),'-*');
hold on;
plot(mean(cumN_norm,1),'-*');
legend({'f','n'})
