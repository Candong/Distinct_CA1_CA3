...
%load file
[behavior_filepaths, beh_temp]=uigetfile('*.mat', 'Chose behavior files to load:','MultiSelect','on');
load([beh_temp behavior_filepaths]);

[cellsort_filepaths, cell_temp]=uigetfile('*.mat', 'Chose cellsort files to load:','MultiSelect','on');
load([cell_temp cellsort_filepaths]);

      prompt = ['is ' cellsort_filepaths(1:12) ' a sparse label data? yes=1,no=0 '  ];
      is_sparse= input(prompt);
      if is_sparse==1
          openfig([cell_temp cellsort_filepaths(1:end-3) 'fig']);
          prompt2 = ['type in cell body id in []'  ];
          cellbody_id= input(prompt2);
      end
      prompt = ['How long is the track? in cm '  ];
      tunnellength= input(prompt);
...
Fc3_DF=data.Fc3(startF:endF,cellbody_id);
Fc2=data.Fc(startF:endF,cellbody_id);
...
ybinned=ybinned';
E=bwlabel(double_thresh(ybinned,0.0165,0.01)); %labels each traversal
w=NaN(200,2);
...
%     binMean(isnan(binMean))=0;
    ...
    minrate=0.2;%original =.3 %percentage of times a tranisent must occur in the PF
    ...
    for tt=1:num_potential_PFs
        ...
        if ((max(temp1(find(L==tt)))))>minDF
            ...
            width=length(find(L==tt))*binsize*(tunnellength/(trackend-trackstart));
            %width_M(tt,ii)=width;
            ...
            for hh=1:numrand
                ...
                for p=1:max(L_shuffle)
                    if mean(numsigs_pos_rew(find(L_shuffle==p)))>minrate
                        width=lengthfind(L_shuffle==p))*binsize*(tunnellength/(trackend-trackstart));
                        %width=(((trackend-trackstart)/100)*length(find(L_shuffle==p)))*(tunnellength/(trackend-trackstart));%for 300cm track, converts to cm
...
if is_sparse==1
save([behavior_filepaths(1:36) '_sparse_PF'],'sig_PFs','sig_PFs_with_noise','PF_width','PF_rate','PF_start_bins','PF_PVALS','PF_end_bins','number_of_PFs','cellbody_id')%,'-append');
else
save([behavior_filepaths(1:36) '_PF'],'sig_PFs','sig_PFs_with_noise','PF_width','PF_rate','PF_start_bins','PF_PVALS','PF_end_bins','number_of_PFs')%,'-append');
end
