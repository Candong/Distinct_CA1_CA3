...
outfile = [beh_filepaths{1}(1:end-4) '_ds'];
if ~iscell(beh_filepaths), beh_filepaths = {beh_filepaths}; end
...
si={whole_data.Tinterval*10000000};%sample interval in usec;
...
for i=1:numel(data)% For each .mat file
    ...
    if correct_frames
        data(i).A1 = data(i).on_t(1);
        data(i).A2 = data(i).on_t(end);
        data(i).framerateA = (data(i).A2 - data(i).A1)/numframes(i);
        data(i).B = round(linspace(data(i).A1, data(i).A2, numframes(i)+1))';
        data(i).on_t = data(i).B;
        data(i).off_t = data(i).B+1;
        %Fill in Image sync signal
        data(i).Image_sync(data(i).on_t) = 4.8;
    end
