function [shift_dist_M shuffle_end shuffle_start]=find_shuffle_dist(shuffle_num,sig_PFs,common_PC_id,PF_start_bins,PF_end_bins,numbins,all_bins,ave_bin)
...
for i =1:size(sig_PFs,2)
    for p=1:size(sig_PFs,1)
        ...
        if ~isempty(common_PC_id{p,i})
            ...
            shuffle_end5=[shuffle_end5 endCOM];
            meanCOM_last5{p,i}=endCOM;
            norm_COMshift{p,i}=(beginCOM-endCOM)/size(sig_PFs{p,i}{1,common_PC_id{p,i}(j)},2);
            norm_from_startCOMshift{p,i}=(beginCOM-endCOM)./(size(sig_PFs{p,i}{1,common_PC_id{p,i}(j)},2)-lap_Start);
            ...
            total_norm_shift=[total_norm_shift norm_act_COMshift{p,i}];
        end
...
shift_dist_M=[shift_dist_M; total_norm_shift];
shuffle_start=[shuffle_start;shuffle_start5];
shuffle_end=[shuffle_end;shuffle_end5];
