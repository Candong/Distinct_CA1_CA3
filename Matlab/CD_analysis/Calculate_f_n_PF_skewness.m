...
[f_x f_y f_x_M f_y_M]=linreg_for_bylap_PF_skewness(f_sig_PFs,f_pf_id,stepsize,standardlap,onsetlap,stoplap);
[n_x n_y n_x_M n_y_M]=linreg_for_bylap_PF_skewness(n_sig_PFs,n_pf_id,stepsize,standardlap,onsetlap,stoplap);
...
%save(['CA1_novel2day_skewnes_step_' num2str(stepsize) '_onsset_' num2str(onsetlap)],'f_x','f_y','n_x','n_y','f_x_M','f_y_M','n_x_M','n_y_M');
 %save(['CA3_fnovel_deltaCOM_' num2str(stoplap)],'f_x','f_y','n_x','n_y','f_jump_y','n_jump_y',...
 % 'f_jump_x','n_jump_x');
 %%
 %figure; histogram(n_y_M(:,1)-n_y_M(:,25))
