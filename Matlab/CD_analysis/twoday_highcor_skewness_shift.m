        ...
        for m=1:size(f_binmean,2)
            for n=1:size(n_binmean,2)
                ...
                if cur_pfcorr>=corr_thresh
                    [f_width f_skewness f_COM f_slope f_significance f_skew_slope skew_sig c_r s_r] = calculate_by_lap_skewness_COM(f_binmean{1},onsetlap,stepsize_com,stepsize_skew,1); % calculate variables for COM
                    ...
                    [n_width n_skewness n_COM n_slope n_significance] = calculate_by_lap_skewness_COM(n_binmean{1},onsetlap,stepsize_com,stepsize_skew,0);
                    ...
                    f_width_removezero=f_width(f_width~=0);
                    n_width_removezero=n_width(n_width~=0);
                    
                    delta_width=[delta_width; f_width_removezero(1) f_width_removezero(end) n_width_removezero(1)];
                    ...
                    slope=[slope f_slope];
                    N_slope=[N_slope n_slope];
                    sig=[sig f_significance];
                    N_sig=[N_sig n_significance];
...
histogram(dd_com(backward_id),-20:1:20,'Normalization','Probability')
line([median(dd_com(backward_id)) median(dd_com(backward_id))], [0,0.2],'color','r') 
legend({'backward pf'})
title('delta com day2 fiirst - day1 last')

[p h]=signrank(dd_com(backward_id))
text(-2,0.1,[ 'signrank p=' num2str(p)]);
text(-2,0.2,[ 'median=' num2str(median(dd_com(backward_id)))]);
...
%histogram(delta_COM(:,3)-delta_COM(:,2),-49.5:1:49.5)
% histogram(dd_com(backward_id),-20:1:20,'Normalization','Probability')
% histogram(dd_com(forward_id),-20:1:20,'Normalization','Probability')
% line([median(dd_com(backward_id)) median(dd_com(backward_id))], [0,0.2],'color','b') 
% line([median(dd_com(forward_id)) median(dd_com(forward_id))], [0,0.2],'color','r') 
% legend({'backward pf'})
% title('delta com day2 fiirst - day1 last')

% [p h]=signrank(dd_com(backward_id))
% text(-2,0.1,[ 'back signrank p=' num2str(p)]);
% text(-2,0.2,[ 'back median=' num2str(median(dd_com(backward_id)))]);
% 
% [p h]=signrank(dd_com(forward_id))
% text(-2,0.1,[ 'fore signrank p=' num2str(p)]);
% text(-2,0.2,[ 'fore median=' num2str(median(dd_com(forward_id)))]);

subplot(1,2,1)
hold on;
plot(day1_last(forward_id),day2_first(forward_id),'o')
plot(day1_last(backward_id),day2_first(backward_id),'o')
...
hold on;
plot([0 50],[0 50])
[p h stats]=signrank(dd_com(backward_id))
text(10,40,[ 'back signrank p=' num2str(p)]);
text(10,35,[ 'back median=' num2str(median(dd_com(backward_id)))]);

[p h stats]=signrank(dd_com(forward_id))
text(30,10,[ 'fore signrank p=' num2str(p)]);
text(30,5,[ 'fore median=' num2str(median(dd_com(forward_id)))]);


subplot(1,2,2)
hold on;
plot(day1_first(forward_id),day2_first(forward_id),'o')
plot(day1_first(backward_id),day2_first(backward_id),'o')
legend({'sig forward', 'sig backward'})
xlabel('day1 first lap')
ylabel('day2 first lap')
hold on;
plot([0 50],[0 50])
[p h]=signrank(d2_com(backward_id))
text(10,40,[ 'back signrank p=' num2str(p)]);
text(10,35,[ 'back median=' num2str(median(d2_com(backward_id)))]);

[p h]=signrank(d2_com(forward_id))
text(30,10,[ 'fore signrank p=' num2str(p)]);
text(30,5,[ 'fore median=' num2str(median(d2_com(forward_id)))]);
...
% histogram(delta_COM(:,3)-delta_COM(:,2),-49.5:2:49.5,'Normalization','Probability')
% line([median(delta_COM(:,3)-delta_COM(:,2)) median(delta_COM(:,3)-delta_COM(:,2))], [0,0.2]) 
% title('delta com all PF')


%%
figure;hold on;
%histogram(delta_COM(:,2)-delta_COM(:,1),-49.5:1:49.5)
histogram(delta_width(:,3)-delta_width(:,2),-49.5:2:49.5,'Normalization','Probability')
line([median(delta_width(:,3)-delta_width(:,2)) median(delta_COM(:,3)-delta_COM(:,2))], [0,0.2],'color','r') 
title('delta width all PF')
text(-2,0.2,[ 'median=' num2str(median(delta_width(:,3)-delta_width(:,2)))]);
%%
...
% hold on
% %histogram(delta_COM(:,3)-delta_COM(:,2),-49.5:1:49.5)
% histogram(delta_skewness(:,3)-delta_skewness(:,2),-3:0.2:3,'Normalization','Probability')
% line([median(delta_skewness(:,3)-delta_skewness(:,2)) median(delta_skewness(:,3)-delta_skewness(:,2))], [0,0.2],'color','r') 
% % legend({'backward pf'})
% title('delta skewness all PF')
% text(-2,0.2,[ 'median=' num2str(median(delta_skewness(:,3)-delta_skewness(:,2)))]);
% 
...
%%
figure;
plot(slope(sig<0.05 & N_sig<0.05)*6,N_slope(sig<0.05 & N_sig<0.05)*6,'o')
hold on;
xlabel( 'nday1 sig slope')
ylabel('nday2  sig slope')
line([-0.5  0.3],[-0.5 0.3])
slope_dif=(slope(sig<0.05 & N_sig<0.05)*6)-(N_slope(sig<0.05 & N_sig<0.05)*6);
[p h stats]=signrank(-slope_dif)
text(0, 0.1, ['signrank p= ' num2str(p)]);
text(-0.1, 0.2, ['day2-day1 median= ' num2str(median(-slope_dif))]);
 %%
% dif_neg_id=d_skewness<-0;
% 
% 
