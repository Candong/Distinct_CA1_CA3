function [x y x_M y_M]=linreg_for_bylap_PF_skewness(f_sig_PFs,pf_id,stepsize,standardlap,onsetlap,stoplap)
     ...
     for p=1:size(f_sig_PFs,1)
       ...
       for j=cur_pf_id'
           ...
           if ~isclipped(binmean)
                ...
                skewness_lap=find_pfskewness(binmean,stepsize);
                
                if lapnum-stepsize+1>=standardlap
                if ~isnan(skewness_lap(standardlap))
                cur_y=skewness_lap;%-skewness_lap(standardlap);
                cur_x=1:lapnum-stepsize+1;
                matrix_y=cur_y;
                matrix_y(skewness_lap==NaN)=NaN;
...
%                     x_M=[x_M; cur_x(1:stoplap)];
                ...
                remove_id=find(isnan(skewness_lap));
                cur_y(remove_id)=[];                    
                cur_x(remove_id)=[];
                y=[y cur_y];
                x=[x cur_x];
