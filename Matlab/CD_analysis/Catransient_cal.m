...
for f=1:size(transient_path_day1,2)
    ...
    for i=1:size(Fc3,2)
        ...
        for j=1:E
            peak=max(Fc3(trans_label==j,i));
            cur_trans=Fc3(trans_label==j,i);
            width=sum(cur_trans>=0.5*peak)/sample_rate;
...
[PF_f_path_day2, temp]=uigetfile('*.mat', 'Chose day2 f PF files to load:','MultiSelect','on');
[PF_n_path_day2, temp]=uigetfile('*.mat', 'Chose day2 n PF files to load:','MultiSelect','on');
...
%%
save([transient_path_day1{1}(1:10) 'transient_info_withPF_new'],'total_frq','total_width','total_peak','PF_total_frq','PF_total_width','PF_total_peak');
